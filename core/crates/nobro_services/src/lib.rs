...
pub mod display {
    ...
    pub struct DisplayCapabilities {
        ...
        pub max_dirty_regions: u16,
        pub buffer_model: DisplayBufferModel,
    }
    ...
    impl DisplayCapabilities {
        ...
        pub fn valid(self) -> bool {
            !self.backend_id.is_empty()
                && self.width != 0
                && self.height != 0
                && self.max_dirty_regions != 0
                && self.frame_bytes().is_some()
        }
    }

    #[derive(Clone, Copy, Debug, PartialEq, Eq)]
    pub struct DisplayMountReceipt {
        pub instance: ServiceInstanceId,
    ...
    pub trait DisplayBackend {
        ...
        fn state(&mut self) -> ServiceState;
        fn mount(&mut self) -> Result<(), ServiceError>;
        fn present(&mut self, frame: &[u8]) -> Result<(), ServiceError>;
        fn quiesce(&mut self) -> Result<(), ServiceError>;
        fn recover(&mut self) -> Result<(), ServiceError>;
    }

    pub struct MountedDisplay<B> {
        backend: B,
        capabilities: DisplayCapabilities,
    }

    impl<B: DisplayBackend> MountedDisplay<B> {
        pub fn mount(
            instance: ServiceInstanceId,
            mut backend: B,
        ) -> Result<(Self, DisplayMountReceipt), (B, ServiceError)> {
            let capabilities = backend.capabilities();
            if !capabilities.valid() {
                return Err((backend, ServiceError::InvalidIdentity));
            }
            if let Err(error) = backend.mount() {
                return Err((backend, error));
            }
            if backend.state() != ServiceState::Ready {
                return Err((backend, ServiceError::BackendFault));
            }
            Ok((
                Self {
                    backend,
        ...
        pub fn present(&mut self, frame: &[u8]) -> Result<PresentReceipt, ServiceError> {
            if self.backend.state() != ServiceState::Ready {
                return Err(ServiceError::NotReady);
            }
            let expected = self
    ...
    mod tests {
        ...
        impl DisplayBackend for Fake {
            fn capabilities(&self) -> DisplayCapabilities {
                DisplayCapabilities {
                    backend_id: "test-mono",
                    width: 8,
                    height: 8,
                    format: PixelFormat::Mono1,
                    max_dirty_regions: 1,
                    buffer_model: DisplayBufferModel::CallerLent,
                }
            }
            ...
            fn present(&mut self, _frame: &[u8]) -> Result<(), ServiceError> {
        ...
        #[test]
        fn exact_frame_size_and_lifecycle_are_enforced() {
            let (mut display, receipt) = MountedDisplay::mount(
            ...
            .ok()
            .unwrap();
            assert_eq!(receipt.capabilities.frame_bytes(), Some(8));
            assert_eq!(display.present(&[0; 7]), Err(ServiceError::InvalidConfig));
            assert_eq!(display.present(&[0; 8]).unwrap().frame_bytes, 8);
            assert_eq!(display.quiesce(), Ok(()));
            assert_eq!(display.present(&[0; 8]), Err(ServiceError::NotReady));
            assert_eq!(display.recover(), Ok(()));
        }
    }
}
