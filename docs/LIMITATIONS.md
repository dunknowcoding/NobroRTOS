...
| Project workflow | `nobro project` creates, explains, builds, simulates, and reports; `nobro firmware` emits a route from one declaration for every maintained exact profile | Four exact nRF52840/RA4M1/SAMD21 profiles produce standalone images; seven exact AVR/ESP8266/ESP32/SAMD21 profiles produce target-built Arduino compositions. Five maintained ports produce an explicit integration plan and reject `--build` because their startup does not yet link arbitrary declarations. Unavailable generic profiles report their board-owned reason. No execution budget, WCET, clock, interrupt, DMA ownership, or provider behavior is inferred |
