...
//
// NobroRTOS firmware exposes local diagnostics through fixed-layout NOBRO_* reports. This example
// uses the library's C ABI header to (1) build a sample report image the way device
// firmware does, (2) match its diagnostic checksum with the same
...
// payload words..., diagnostic-checksum-of-everything-before-it.
static uint32_t demo_report[] = {
    ...
    0,                          // diagnostic checksum (filled in setup)
};
...
void setup() {
  ...
  // Finalize the diagnostic snapshot like firmware: unkeyed XOR over prior words.
  demo_report[WORDS - 1] =
      nobro_report_diagnostic_checksum_words(demo_report, WORDS - 1);
...
void loop() {
  ...
  Serial.print("diag sum   = "); print_hex32(demo_report[WORDS - 1]);
