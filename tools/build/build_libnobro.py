...
"""Tier C packaging: prebuilt `libnobro.a` a C developer links with no Rust installed.
  ...
  --build   cargo-build the nobro-tierc staticlib (nRF52840 no-SoftDevice layout),
            harvest the generated linker scripts (link.x/device.x/memory.x/defmt.x) from the
  ...
  --check   THE GATE: link both reference modules and negative init/poll modules
    ...
    python tools/build/build_libnobro.py --check
"""
import argparse
import glob
import json
...
def build() -> int:
    ...
    for script, package in (
        ("link.x", "cortex-m-rt"),
        ("device.x", "nrf52840-pac"),
        ("defmt.x", "defmt"),
    ):
        hits = [
    ...
    print(
        f"staged {OUT}: libnobro.a + self-contained linker scripts + headers + "
