...
composition, and CI runners must submit all of their required gate receipts.
    ...
    python tools/checks/platforms/check_platform_tiers.py
    python tools/checks/platforms/check_platform_tiers.py --selftest
    python tools/checks/platforms/check_platform_tiers.py --begin-receipts RUNNER
    python tools/checks/platforms/check_platform_tiers.py --run-gate GATE
    python tools/checks/platforms/check_platform_tiers.py --assert-receipts RUNNER
        ...
        with mock.patch.object(subprocess, "run", return_value=mock.Mock(returncode=0)) as run:
            ...
            _expect(
                _quiet_call(
                    execute_gate,
                    good,
                    "arduino-ble-unor4-target-build",
                    receipt_root,
                    source_root,
                )
                == 0,
                "ArduinoBLE UNO R4 gate success",
            )
            _expect(
                _quiet_call(
                    execute_gate,
            ...
            _expect(
                _quiet_call(
                    execute_gate,
                    good,
                    "arduino-samd21-pn532-target-build",
                    receipt_root,
                    source_root,
                )
                == 0,
                "Arduino SAMD21 PN532 gate success",
            )
