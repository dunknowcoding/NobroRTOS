...
class ContractBuilderTests(unittest.TestCase):
    def test_distribution_metadata_points_to_canonical_contracts(self) -> None:
        ...
        self.assertEqual(sdk_manifest["license"], "PolyForm-Noncommercial-1.0.0")
        self.assertEqual(
        ...
        self.assertEqual(platformio["license"], "PolyForm-Noncommercial-1.0.0")
        self.assertEqual(
    ...
    def test_doctor_summarizes_host_and_package_health(self) -> None:
        ...
        self.assertIn("report_matrix_gate", report["host_simulators"])
    ...
    def test_software_surface_gate_composes_release_checks(self) -> None:
        ...
        self.assertIn("report_matrix", report["checks"])
        self.assertTrue(report["checks"]["report_matrix"]["passing"])
    ...
    def test_project_template_repair_restores_stale_python_host_gate_task(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for task in tasks["tasks"]:
                ...
                if task["label"] == "NobroRTOS: Report Matrix Gate":
                    task["args"] = ["-m", "nobro_rtos", "decode-report"]
            tasks_path.write_text(json.dumps(tasks, indent=2), encoding="utf-8")
            ...
            self.assertIn("report matrix gate task command mismatch", before.errors)
            ...
            self.assertIn(
                "watchdog matrix gate task command mismatch",
                report.before_errors,
            )
            self.assertIn(
            ...
            self.assertIn(
                "report matrix gate task command mismatch",
                report.before_errors,
            )
            ...
            watchdog_matrix_gate = next(
                task
                for task in repaired["tasks"]
                if task["label"] == "NobroRTOS: Watchdog Matrix Gate"
            )
            self.assertIn("check-watchdog-matrix", watchdog_matrix_gate["args"])
            scheduler_matrix_gate = next(
                task
                for task in repaired["tasks"]
                if task["label"] == "NobroRTOS: Scheduler Matrix Gate"
            )
            ...
            report_matrix_gate = next(
                task
                for task in repaired["tasks"]
                if task["label"] == "NobroRTOS: Report Matrix Gate"
            )
            self.assertIn("check-report-matrix", report_matrix_gate["args"])
    ...
    def test_report_matrix_covers_status_and_domain_paths(self) -> None:
        ...
        scenarios = {entry["name"]: entry for entry in report["scenarios"]}
        self.assertEqual(
            scenarios["sample_reports_decode_domain_fields"]["decoded"]["runtime"][
                "next_alarm_due_us"
            ],
            0x1234_5678_9ABC,
        )
        self.assertEqual(
            scenarios["sample_reports_decode_domain_fields"]["decoded"]["ai_model"][
                "route_preference"
            ],
            "hybrid_fallback",
        )
        self.assertEqual(
            scenarios["status_classes_are_reported"]["statuses"]["missing"],
            "missing",
        )
        self.assertEqual(
        ...
        self.assertEqual(
            scenarios["failure_labels_are_reported"]["adapter_error_module_label"],
            "bus",
        )

    def test_report_matrix_command_returns_zero(self) -> None:
        with mock.patch(
            "sys.argv",
    ...
    def test_ai_route_matrix_covers_local_edge_remote_and_fallback_paths(self) -> None:
        ...
        self.assertEqual(report["scenario_count"], 7)
        targets = {
            scenario["name"]: scenario["summary"]["target"]
            for scenario in report["scenarios"]
        }
        self.assertEqual(targets["local_on_device"], "on_device")
        self.assertEqual(targets["remote_api_ready"], "remote_api")
        self.assertEqual(targets["edge_sidecar_ready"], "edge_sidecar")
        self.assertEqual(targets["hybrid_prefers_local"], "on_device")
        self.assertEqual(
            targets["remote_circuit_uses_stale_snapshot"],
            "stale_snapshot",
        )
        self.assertEqual(targets["hybrid_budget_degrades"], "degraded_fallback")
        self.assertEqual(targets["local_only_unavailable"], "unavailable")

    def test_ai_route_matrix_command_returns_zero(self) -> None:
        with mock.patch(
            "sys.argv",
    ...
    def test_watchdog_matrix_covers_liveness_paths(self) -> None:
        ...
        self.assertEqual(report["scenario_count"], 5)
        scenarios = {entry["name"]: entry for entry in report["scenarios"]}
        self.assertEqual(scenarios["non_mutating_precheck"]["expired_count"], 1)
        self.assertEqual(
            scenarios["non_mutating_precheck"]["entry"]["missed"],
            0,
        )
        self.assertEqual(
            scenarios["expiry_updates_missed"]["entry"]["missed"],
            1,
        )
        self.assertEqual(
            scenarios["heartbeat_resets_missed"]["entry"]["last_beat_us"],
            160,
        )
        self.assertEqual(
            scenarios["multi_module_selective_expiry"]["expired"][0]["module"],
            "sensor",
        )
        self.assertEqual(
            scenarios["capacity_exhaustion_is_reported"]["error_label"],
            "watchdog capacity exhausted",
        )

    def test_watchdog_matrix_command_returns_zero(self) -> None:
        with mock.patch(
            "sys.argv",
    ...
    def test_scheduler_matrix_covers_deadline_paths(self) -> None:
        ...
        self.assertEqual(report["scenario_count"], 6)
        scenarios = {entry["name"]: entry for entry in report["scenarios"]}
        self.assertEqual(scenarios["on_time_ticks"]["deadline_misses"], 0)
        self.assertEqual(
            scenarios["early_late_within_tolerance"]["max_jitter_us"],
            20,
        )
        self.assertEqual(
            scenarios["late_ticks_miss_deadline"]["deadline_misses"],
            2,
        )
        self.assertEqual(scenarios["u32_wraparound"]["max_jitter_us"], 3)
        self.assertEqual(
            scenarios["reset_clears_counters"]["after"]["deadline_misses"],
            0,
        )
        self.assertEqual(
            scenarios["invalid_config_is_rejected"]["period_error"],
            "deadline_period_us must be positive",
        )

    def test_scheduler_matrix_command_returns_zero(self) -> None:
        with mock.patch(
            "sys.argv",
