...
def _vscode_tasks_json(target: ProjectTarget) -> str:
    ...
    if target == ProjectTarget.PYTHON_HOST:
        ...
        tasks.append(
            {
                "label": "NobroRTOS: AI Preflight Matrix Gate",
                "type": "shell",
                "command": "python",
                "args": [
                    "-m",
        ...
        tasks.append(
            {
                "label": "NobroRTOS: Watchdog Matrix Gate",
                "type": "shell",
                "command": "python",
                "args": [
                    "-m",
        ...
        tasks.append(
            {
                "label": "NobroRTOS: Scheduler Matrix Gate",
                "type": "shell",
                "command": "python",
                "args": [
                    "-m",
        ...
        tasks.append(
            {
                "label": "NobroRTOS: Event Log Matrix Gate",
                "type": "shell",
                "command": "python",
                "args": [
                    "-m",
        ...
        tasks.append(
            {
                "label": "NobroRTOS: Degrade Matrix Gate",
                "type": "shell",
                "command": "python",
                "args": [
                    "-m",
        ...
        tasks.append(
            {
                "label": "NobroRTOS: Boot Summary Matrix Gate",
                "type": "shell",
                "command": "python",
                "args": [
                    "-m",
        ...
        tasks.append(
            {
                "label": "NobroRTOS: Bundle Matrix Gate",
                "type": "shell",
                "command": "python",
                "args": [
                    "-m",
        ...
        tasks.append(
            {
                "label": "NobroRTOS: Report Matrix Gate",
                "type": "shell",
                "command": "python",
                "args": [
                    "-m",
...
def _validate_vscode_tasks(root: Path, target: ProjectTarget) -> list[str]:
    ...
    ai_preflight_matrix_gate_task = _task_by_label(
        tasks,
        "NobroRTOS: AI Preflight Matrix Gate",
    )
    if target == ProjectTarget.PYTHON_HOST and ai_preflight_matrix_gate_task is None:
        errors.append("missing NobroRTOS: AI Preflight Matrix Gate task")
    elif target == ProjectTarget.PYTHON_HOST and not _task_has_args(
        ai_preflight_matrix_gate_task,
    ...
    watchdog_matrix_gate_task = _task_by_label(
        tasks,
        "NobroRTOS: Watchdog Matrix Gate",
    )
    if target == ProjectTarget.PYTHON_HOST and watchdog_matrix_gate_task is None:
        errors.append("missing NobroRTOS: Watchdog Matrix Gate task")
    elif target == ProjectTarget.PYTHON_HOST and not _task_has_args(
        watchdog_matrix_gate_task,
        ("-m", "nobro_rtos", "check-watchdog-matrix"),
    ):
        errors.append("watchdog matrix gate task command mismatch")

    scheduler_matrix_gate_task = _task_by_label(
        tasks,
        "NobroRTOS: Scheduler Matrix Gate",
    )
    if target == ProjectTarget.PYTHON_HOST and scheduler_matrix_gate_task is None:
        errors.append("missing NobroRTOS: Scheduler Matrix Gate task")
    elif target == ProjectTarget.PYTHON_HOST and not _task_has_args(
        scheduler_matrix_gate_task,
        ("-m", "nobro_rtos", "check-scheduler-matrix"),
    ):
        errors.append("scheduler matrix gate task command mismatch")

    event_log_matrix_gate_task = _task_by_label(
        tasks,
        "NobroRTOS: Event Log Matrix Gate",
    )
    if target == ProjectTarget.PYTHON_HOST and event_log_matrix_gate_task is None:
        errors.append("missing NobroRTOS: Event Log Matrix Gate task")
    elif target == ProjectTarget.PYTHON_HOST and not _task_has_args(
        event_log_matrix_gate_task,
    ...
    degrade_matrix_gate_task = _task_by_label(
        tasks,
        "NobroRTOS: Degrade Matrix Gate",
    )
    if target == ProjectTarget.PYTHON_HOST and degrade_matrix_gate_task is None:
        errors.append("missing NobroRTOS: Degrade Matrix Gate task")
    elif target == ProjectTarget.PYTHON_HOST and not _task_has_args(
        degrade_matrix_gate_task,
    ...
    boot_summary_matrix_gate_task = _task_by_label(
        tasks,
        "NobroRTOS: Boot Summary Matrix Gate",
    )
    if target == ProjectTarget.PYTHON_HOST and boot_summary_matrix_gate_task is None:
        errors.append("missing NobroRTOS: Boot Summary Matrix Gate task")
    elif target == ProjectTarget.PYTHON_HOST and not _task_has_args(
        boot_summary_matrix_gate_task,
        ("-m", "nobro_rtos", "check-boot-summary-matrix"),
    ):
        errors.append("boot summary matrix gate task command mismatch")

    bundle_matrix_gate_task = _task_by_label(
        tasks,
        "NobroRTOS: Bundle Matrix Gate",
    )
    if target == ProjectTarget.PYTHON_HOST and bundle_matrix_gate_task is None:
        errors.append("missing NobroRTOS: Bundle Matrix Gate task")
    elif target == ProjectTarget.PYTHON_HOST and not _task_has_args(
        bundle_matrix_gate_task,
        ("-m", "nobro_rtos", "check-bundle-matrix"),
    ):
        errors.append("bundle matrix gate task command mismatch")

    report_matrix_gate_task = _task_by_label(
        tasks,
        "NobroRTOS: Report Matrix Gate",
    )
    if target == ProjectTarget.PYTHON_HOST and report_matrix_gate_task is None:
        errors.append("missing NobroRTOS: Report Matrix Gate task")
    elif target == ProjectTarget.PYTHON_HOST and not _task_has_args(
        report_matrix_gate_task,
        ("-m", "nobro_rtos", "check-report-matrix"),
    ):
        errors.append("report matrix gate task command mismatch")
